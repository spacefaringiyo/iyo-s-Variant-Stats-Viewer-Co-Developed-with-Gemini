...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.load_button = customtkinter.CTkButton(path_frame, text="Load Stats", font=("Arial", 16, "bold"), height=40, command=self.load_stats_thread); self.load_button.grid(row=1, column=0, columnspan=2, pady=(0, 10), sticky="ew")
        ...
        self.display_options_frame = customtkinter.CTkFrame(analysis_tools_frame); self.display_options_frame.grid(row=0, column=0, sticky="ew"); analysis_tools_frame.grid_columnconfigure(0, weight=1)
        self.filters_frame = customtkinter.CTkFrame(analysis_tools_frame); self.filters_frame.grid(row=1, column=0, sticky="ew", pady=(5,0)); analysis_tools_frame.grid_columnconfigure(0, weight=1)
        
        self.display_options_frame.grid_columnconfigure((0,1,2), weight=1)
        ...
        misc_group = customtkinter.CTkFrame(self.display_options_frame); misc_group.grid(row=0, column=2, padx=5, pady=5, sticky="ew")
    ...
    def display_grid_data(self):
        ...
        summary_cols_map = {'BEST Score': 'Best', 'BEST CM': 'cm', '% vs Base': '%'}
        ...
        final_col_order = ['Scenario'] + sens_cols + other_cols + list(summary_cols_map.values())
    ...
    def apply_highlighting(self, grid_data):
        ...
        heatmap_cols = [c for c in grid_data.columns if self.is_float(c)]
    ...
    def get_heatmap_color(self, normalized_value):
        COLOR_RED = (120, 47, 47)
        COLOR_YELLOW = (122, 118, 50)
        COLOR_GREEN = (54, 107, 54)
    ...
    def bind_hide_events(self, table_values):
        if not self.results_table or not table_values:
            return
            
        column_headers = table_values[0]
        for j, header_text in enumerate(column_headers):
            cm_value = header_text.replace('cm', '')
            if self.is_float(cm_value):
                cell_widget = self.results_table.frame[0, j]
                cell_widget.bind("<Button-3>", lambda e, cm=cm_value: self.show_col_context_menu(e, cm))
        ...
        for i, row_data in enumerate(data_rows):
            if row_data:
                ...
                if scenario_name:
                    cell_widget = self.results_table.frame[i + 1, 0]
                    cell_widget.bind("<Button-3>", lambda e, s=scenario_name: self.show_row_context_menu(e, s))

    def show_col_context_menu(self, event, cm_value):
        menu = tkinter.Menu(self, tearoff=0)
        menu.add_command(label=f"Hide {cm_value}cm", command=lambda: self.hide_cm(cm_value))
        menu.tk_popup(event.x_root, event.y_root)

    def show_row_context_menu(self, event, scenario_name):
        menu = tkinter.Menu(self, tearoff=0)
        menu.add_command(label=f"Hide Scenario", command=lambda: self.hide_scenario(scenario_name))
        menu.tk_popup(event.x_root, event.y_root)

    def hide_cm(self, cm_value):
        self.hidden_cms.add(str(cm_value))
        self.save_user_data()
        self.display_grid_data()

    def hide_scenario(self, scenario_name):
        self.hidden_scenarios.add(scenario_name)
        self.save_user_data()
        self.build_filters_and_get_data()

    def open_manage_hidden_window(self):
        win = customtkinter.CTkToplevel(self)
        win.title("Manage Hidden Items")
        ...
        tabview.add("Hidden Scenarios")
        tabview.add("Hidden CMs")
    ...
    def _populate_manage_hidden_window(self, tabview):
        ...
        scenarios_frame = customtkinter.CTkScrollableFrame(tabview.tab("Hidden Scenarios"))
        scenarios_frame.pack(expand=True, fill="both")
        if not self.hidden_scenarios:
             customtkinter.CTkLabel(scenarios_frame, text="No hidden scenarios.").pack(pady=10)
        for scenario in sorted(list(self.hidden_scenarios)):
            item_frame = customtkinter.CTkFrame(scenarios_frame)
            item_frame.pack(fill="x", pady=2)
            customtkinter.CTkLabel(item_frame, text=scenario, wraplength=400, justify="left").pack(side="left", padx=5, pady=2)
            unhide_btn = customtkinter.CTkButton(item_frame, text="Unhide", width=80, command=lambda s=scenario: self.unhide_item('scenario', s, tabview))
            unhide_btn.pack(side="right", padx=5)

        cms_frame = customtkinter.CTkScrollableFrame(tabview.tab("Hidden CMs"))
        cms_frame.pack(expand=True, fill="both")
        if not self.hidden_cms:
             customtkinter.CTkLabel(cms_frame, text="No hidden cm values.").pack(pady=10)
        for cm in sorted(list(self.hidden_cms), key=float):
            item_frame = customtkinter.CTkFrame(cms_frame)
            item_frame.pack(fill="x", pady=2)
            customtkinter.CTkLabel(item_frame, text=f"{cm}cm").pack(side="left", padx=5, pady=2)
            unhide_btn = customtkinter.CTkButton(item_frame, text="Unhide", width=80, command=lambda c=cm: self.unhide_item('cm', c, tabview))
            unhide_btn.pack(side="right", padx=5)

    def unhide_item(self, item_type, value, tabview):
        """Removes an item from the hidden set and intelligently refreshes the app."""
        if item_type == 'scenario':
            self.hidden_scenarios.remove(value)
        elif item_type == 'cm':
            self.hidden_cms.remove(str(value))
    ...
    def on_load_complete(self, result_df):
        ...
        if result_df is not None and not result_df.empty:
            ...
            if self.scenario_search_var.get():
                self.current_family_info = None; self.update_grid()
        else:
    ...
    def build_filters_and_get_data(self):
        ...
        if self.hidden_scenarios:
            filtered_family_info = filtered_family_info[~filtered_family_info['Scenario'].isin(self.hidden_scenarios)]
        ...
        if not all_modifiers:
            ...
            self.current_unfiltered_grid_data = filtered_family_info[filtered_family_info['Scenario'] == self.scenario_search_var.get()]
            self.display_grid_data()
            return
        self.filters_frame.grid()
        customtkinter.CTkLabel(self.filters_frame, text="Compare by:").pack(side="left", padx=(10,5), pady=5)
        if not self.variable_axis_var.get() or self.variable_axis_var.get() not in all_modifiers.keys():
            self.variable_axis_var.set(list(all_modifiers.keys())[0])
        for key in all_modifiers.keys():
            rb = customtkinter.CTkRadioButton(self.filters_frame, text=key, variable=self.variable_axis_var, value=key, command=self.build_filters_and_get_data); rb.pack(side="left", padx=5, pady=5)
        base_scenario = self.scenario_search_var.get()
        self.current_unfiltered_grid_data = engine.analyze_variants(filtered_family_info, variable_axis=self.variable_axis_var.get(), fixed_filters={}, base_scenario=base_scenario)
        self.display_grid_data()
    ...
    def load_stats_thread(self):
        ...
        thread = threading.Thread(target=self.perform_load, args=(stats_path,))
        thread.daemon = True
        thread.start()
